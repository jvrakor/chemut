...
#' @importFrom dplyr select mutate filter contains
...
snv <- function(input_file = NULL, ref_file = system.file("extdata", package="chemut"), parental_strain = NULL, mutant_strain = NULL,
    ...
    for(rd in read_depth){
        ...
        # Exclude chr2 duplication 2263132 to 3015703
        chr2 <- vcf_$CHROM == "chr2" & vcf_$POS >= 2263132 & vcf_$POS <= 3015703
        vcf_ <- vcf_[!chr2,]
       ...
       # variants with >= 2 alternative alleles were saved to variant_mult_alt.txt
        ...
        if(dim(vcf_multi_alt_split)[1]>0){
            for(n_ in names_all){
                ...
                vcf_tmp[,alt_freq_n] <- dplyr::select(alleles_freq/alleles_freq$sum, -dplyr::contains("ref"), -sum)
        ...
        ref_freq <- dplyr::select(vcf_filter2, dplyr::contains("ref_freq")) # columns of ref_freq
        ref_freq_n <- colnames(ref_freq)
        # All strains have freqency >= ref_thld (default=0.8) or frequency <= var_thld (default=0.2).
        vcf_filter3_1 <- vcf_filter2[rowSums((ref_freq <= var_thld | ref_freq >= ref_thld)) == dim(ref_freq)[2],]
        ...
        if(dim(vcf_multi_alt)[1]>0){
            alt_freq <- dplyr::select(vcf_multi_alt_split, dplyr::contains("_freq")) # columns of ref_freq
        ...
        vcf_filter3_2 <- c()
        ref_freq <- dplyr::select(vcf_filter3_1, dplyr::contains("ref_freq"))
        ref_freq_par <- dplyr::select(ref_freq, dplyr::contains(names_par)) # frequency of the reference allele in the parental strain
        depth_ <- dplyr::select(vcf_filter3_1,dplyr::contains("_DP"))
        depth_par <- dplyr::select(depth_,dplyr::contains(names_par)) # read depth in the parental strain

        for(n_ in names_mut){
            ref_freq_tmp <- dplyr::select(ref_freq, dplyr::contains(n_)) # frequency of the reference allele in a given mutant strain
            depth_tmp <- dplyr::select(depth_,dplyr::contains(n_)) # read depth in a given mutant strain
            freq_diff_l <- abs(ref_freq_par - ref_freq_tmp) >= freq_diff # the difference in allele frequency between the mutant and parental strains
        ...
        vcf_multiple_alt <- c()
        if(dim(vcf_multi_alt_split)[1]>0){
            alt_freq <- dplyr::select(vcf_multi_alt_split, dplyr::contains("_freq"))
            alt_freq_par <- dplyr::select(alt_freq, dplyr::contains(names_par)) # frequency of the reference allele in the parental strain
            depth_ <- dplyr::select(vcf_multi_alt_split,dplyr::contains("_DP"))
            depth_par <- dplyr::select(depth_,dplyr::contains(names_par)) # read depth in the parental strain
            for(n_ in names_mut){
                alt_freq_tmp <- dplyr::select(alt_freq, dplyr::contains(n_)) # frequency of the reference allele in a given mutant strain
                depth_tmp <- dplyr::select(depth_,dplyr::contains(n_)) # read depth in a given mutant strain
        ...
        if(is.null(vcf_filter3_2)){
            ...
            if(dim(gene_list)[1]>0){
                ...
                # Count incidence of GAorTC mutations
